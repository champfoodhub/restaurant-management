...
/**
 * Toggle seasonal menu active status
 */
export async function toggleSeasonalMenuActive(id: string, isActive: boolean): Promise<void> {
  const database = getDatabase();
  
  await database.runAsync(
    'UPDATE seasonal_menus SET isActive = ?, updatedAt = ? WHERE id = ?',
...
export default {
  ...
  updateSeasonalMenu,
  deleteSeasonalMenu,
  getActiveSeasonalMenus,
  toggleSeasonalMenuActive,
};
